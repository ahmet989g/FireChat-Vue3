...
import { ref as fireRef, set, push, onValue } from "firebase/database";
  import { db } from "./db";
...
export default {
  setup(){
    ...
    const SendMessage = () => {
      ...
      const messageRef = set(push(fireRef(db, "messages")), message);
    ...
    onMounted(() => {
      const messagesRef = fireRef(db, '/messages');
